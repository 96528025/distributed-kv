...
- Local Raft snapshot creation and follower Raft snapshot installation.
...
current complete CI gate is nine suites and 152 checks on each supported Python version;
...
| `distributed_kv_snapshot_operations_total` | counter | `shard`, `operation` | Successful Raft snapshot `create` or follower `install`; not storage checkpoints |
