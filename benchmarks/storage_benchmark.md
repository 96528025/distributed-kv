...
python3 benchmark_storage.py --quick --no-save  # 冒烟且不生成结果文件
...
## 保留的来源分支结果（2026-07-23，单机 macOS，Python 3.14.6，fsync=off）

以下原始 JSON/CSV 来自 `df874e6`，作为迁移来源的可审计历史结果保留。当前集成版
进一步修复了 partial-tail 后续 append 和 restart index continuity，并让
`rotate_records` 按实际 WAL record 数而不是 commit 调用次数计数；因此单写结论不变，
但 batching 行的 checkpoint 时机可能与这份历史数据不同，应以重新运行脚本的结果为准。
