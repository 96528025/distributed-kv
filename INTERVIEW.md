...
> 约 30 ops/s、p50 涨到 30ms 以上。我加了一层 WAL：每次提交只**追加一条定长帧记录**（O(1)），
...
WAL 单写 p50 不随规模变（macOS 恒为 0.0083ms，Linux 恒为 0.0072ms）；而它的总吞吐在
5 万条时明显回落（macOS 8.6 万 → 2.9 万 ops/s，Linux 7.3 万 → 1.6 万 ops/s），
...
- **新增 WAL 测试**：`python3 test_wal.py` → 17/17，含真实 3 节点 `SIGKILL` 后恢复。
- 所有测试用临时目录，跑完不留残留文件。

### 基准数字（两个平台交叉验证过）

复现命令：`python3 benchmark_storage.py`（每点 1000 次写，重复 3 次取中位数）。
原始数据在 `benchmarks/storage_results_*.json`。

单条 commit，随 store 规模变化：

| store 规模 | JSON p50 | WAL p50 | JSON 吞吐 | WAL 吞吐 | 倍数 |
|---|---|---|---|---|---|
| 100 | 0.47 ms | **0.0072 ms** | 2080 ops/s | 72.7k ops/s | 35× |
| 1,000 | 1.02 ms | **0.0072 ms** | 967 ops/s | 68.2k ops/s | 71× |
| 10,000 | 6.25 ms | **0.0072 ms** | 155 ops/s | 49.9k ops/s | 322× |
| 50,000 | 30.06 ms | **0.0072 ms** | 32 ops/s | 16.1k ops/s | 496× |

<sub>Linux / Python 3.11.15。macOS / Python 3.14.6 上同一套脚本：WAL p50 恒为 0.0083ms，
JSON 0.72 → 35.3ms，50k 处倍数 1018×。</sub>

**能拿去讲的是哪一条，要分清楚：**

- ✅ **WAL 单写 p50 与数据量无关** —— 数据量涨 500×（100 → 50,000 条），p50 一动不动。
  Linux 四个规模全是 0.0072ms，macOS 全是 0.0083ms。这是 O(1) 的直接证据，两个平台、
  两个 Python 版本都成立，**这是最硬的一条**。
- ✅ **JSON 单写 p50 随数据量线性劣化** —— 0.47 → 30.06 ms，约 64×。写放大 O(N) 的直接证据。
- ⚠️ **倍数（496× / 1018×）不要单独引用。** 它在 macOS 上约 1000×、Linux 上约 500×，
  差了一倍——因为分母（JSON 吞吐）本身就受文件系统和页缓存行为影响很大。
  要讲就讲「同一台机器上，50k 规模下 WAL 比全量重写快约 500×（Linux）/ 1000×（macOS）」，
  **带上平台**；或者干脆只讲上面那两条 p50 结论，它们不依赖平台。

如果面试官追问「你这个 benchmark 可信吗」，主动说这几点：这是**单机存储层微基准**，
刻意排除了 Raft / HTTP / GIL 的噪声，只测持久化路径；每点重复 3 次取中位数，
min/max 一并记在 JSON 里；跨平台跑过，结论中**只有 p50 的规模无关性是稳定的**，
吞吐倍数会随环境浮动一倍以上，所以我不会把倍数写成一个孤立的数字。
